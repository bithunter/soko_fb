(function() {
    ...
    var s = e.helpers = {},
        ...
        C = (s.calculateScaleRange = function(t, i, e, s, n) {
            ...
            for (var c = Math.abs(l - r), u = y(c), d = Math.ceil(l / (1 * Math.pow(10, u))) * Math.pow(10, u), p = s ? 0 : Math.floor(r / (1 * Math.pow(10, u))) * Math.pow(10, u), f = d - p, m = Math.pow(10, u), x = Math.round(f / m); (x > a || 2 * x < a) && !h;)
                if (x > a)
                    m *= 2, x = Math.round(f / m), x % 1 !== 0 && (h = !0);
                else if (n && u >= 0) {
                    if (m / 2 % 1 !== 0)
                        break;
    ...
    e.instances = {}, e.Type = function(t, i, s) {
        ...
        buildYLabels: function() {
            this.yLabels = [];
            for (var t = m(this.stepValue), i = 0; i <= this.steps; i++)
